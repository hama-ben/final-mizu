...
const authProtectedRouter: IRouter = Router();
...
interface PendingRegistration {
  ...
  commune: string;
  referralCode?: string;
  referralSource?: "referral" | "social_share";
}
...
const REFERRAL_CODE_PATTERN = /^[A-Z0-9]{6,8}$/;
...
async function findReferralReferrer(code: string): Promise<{ id: string; userType: string } | null> {
...
function referralCodeConflict(error: unknown): boolean {
  ...
  return code === "23505" && constraint.includes("referral");
}

// ─────────────────────────────────────────────────────────────────────────────
// Public referral-code validation used by registration's live field check.
// The final registration step validates the code again before accepting it.
// ─────────────────────────────────────────────────────────────────────────────
router.get("/auth/referral-code/validate", async (req, res): Promise<void> => {
  const code = normalizeReferralCode(req.query.code);
  const userType = req.query.userType === "مستهلك" || req.query.userType === "سائق"
    ? req.query.userType
    : null;

  if (!code || !REFERRAL_CODE_PATTERN.test(code)) {
    res.json({ valid: false, error: "رمز الدعوة يجب أن يكون من 6 إلى 8 أحرف أو أرقام" });
    return;
  }

  try {
    const referrer = await findReferralReferrer(code);
    if (!referrer) {
      res.json({ valid: false, error: "رمز الدعوة غير موجود" });
      return;
    }
    if (userType === "مستهلك" && referrer.userType !== "سائق") {
      res.json({ valid: false, error: "لا يمكن للمستهلك استخدام رمز مستهلك آخر" });
      return;
    }

    res.json({ valid: true, referrerRole: referrer.userType });
  } catch (err) {
    const { status, message } = handleDbError(err, "referral-code validation");
...
router.post("/auth/register-request", async (req, res): Promise<void> => {
  ...
  const referralSource = req.body?.referralSource === "social_share"
    ? "social_share"
    : "referral";
  ...
  if (req.body?.referralCode && (!referralCode || !REFERRAL_CODE_PATTERN.test(referralCode))) {
  ...
  if (referralCode) {
    try {
      const referrer = await findReferralReferrer(referralCode);
      if (!referrer) {
        res.status(400).json({ error: "رمز الدعوة غير موجود" });
        return;
      }
      if (userType === "مستهلك" && referrer.userType !== "سائق") {
        res.status(400).json({ error: "لا يمكن للمستهلك استخدام رمز مستهلك آخر" });
        return;
      }
    } catch (err) {
      const { status, message } = handleDbError(err, "register-request referral lookup");
      res.status(status).json({ error: message });
      return;
    }
  }
...
router.post("/auth/verify-otp", async (req, res): Promise<void> => {
  ...
  let referralReferrer: { id: string; userType: string } | null = null;
  if (pending.referralCode) {
    try {
      referralReferrer = await findReferralReferrer(pending.referralCode);
      if (!referralReferrer || (pending.userType === "مستهلك" && referralReferrer.userType !== "سائق")) {
        res.status(400).json({ error: "رمز الدعوة لم يعد صالحاً لهذا النوع من الحسابات" });
        return;
      }
    } catch (err) {
      const { status, message } = handleDbError(err, "verify-otp referral lookup");
      res.status(status).json({ error: message });
      return;
    }
  }
  ...
  for (let attempt = 0; attempt < 5 && !user; attempt += 1) {
    try {
      user = await db.transaction(async (tx) => {
        const [inserted] = await tx
          ...
          .values({
            ...
            accountStatus,
            referralCode:  generateReferralCode(pending.name),
            referredBy:    referralReferrer?.id ?? null,
            createdAt:     new Date(),
          })
          .onConflictDoUpdate({
        ...
        if (!inserted) throw new Error("User row was not created");
        ...
        if (referralReferrer) {
          await tx
            ...
            .values({
              ...
              referredRole: pending.userType,
              source: pending.referralSource ?? "referral",
              status: "pending",
            })
            .onConflictDoNothing();
        }
      ...
      if (referralCodeConflict(err) && attempt < 4) continue;
      const { status, message } = handleDbError(err, "verify-otp insert user and referral");
      res.status(status).json({ error: message });
      return;
    }
  }
...
// ─────────────────────────────────────────────────────────────────────────────
authProtectedRouter.get("/auth/devices", async (req, res): Promise<void> => {
...
authProtectedRouter.delete("/auth/devices/:deviceId", async (req, res): Promise<void> => {
  const userId = req.auth!.userId;

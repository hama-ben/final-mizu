...
import { eq, desc, sql, asc, and, ne, isNull } from "drizzle-orm";
...
import { closeContestsForApprovedDriverThreshold } from "../lib/no-driver-contest";
import { discountAmountFor } from "./wheel";

const router: IRouter = Router();
const FIRST_APPROVAL_TRIAL_DAYS = 90;
...
function requireAdmin(req: Request, res: Response, next: NextFunction): void {
  ...
  if (typeof provided !== "string" || !safeCompare(provided, adminKey)) {
...
router.get("/admin/drivers", async (req, res): Promise<void> => {
  ...
  const signedRows = await Promise.all(
    rows.map(async (r) => ({
      ...
      truckFrontPhotoUrl: await signDriverDocUrl(r.truckFrontPhotoUrl),
      driverLicenseUrl:   await signDriverDocUrl(r.driverLicenseUrl),
      truckSidePhotoUrl:  await signDriverDocUrl(r.truckSidePhotoUrl),
      truckVideoUrl:      await signDriverDocUrl(r.truckVideoUrl),
    }))
  );
...
router.post("/admin/drivers/:driverId/approve", async (req, res): Promise<void> => {
  ...
  if (user.userType !== "سائق") {
    res.status(400).json({ error: "هذا المسار مخصص للسائقين فقط" });
    return;
  }

  const isFirstApproval = !user.firstApprovalGranted;
  const now = new Date();
  const giftDays = FIRST_APPROVAL_TRIAL_DAYS;
  ...
  const [updated] = await db
    ...
    .set({
      ...
      ...(isFirstApproval
        ? {
            // Guarantee 90 days from this approval while preserving any longer
            // subscription already on the account. The SQL expression is atomic
            // and the firstApprovalGranted predicate below prevents double grants.
  ...
  if (isFirstApproval) {
    ...
    req.log.info({ driverId, subscriptionExpiresAt: updated.subscriptionExpiresAt, giftDays }, "Driver approved (first time) — 90-day gift granted");
  } else {
    await insertTargetedAnnouncement(
      driverId,
      "تم قبولك بيننا",
  ...
  try {
    await qualifyReferralForUser(driverId, "driver_approved");
  } catch (error) {
    // Approval must remain successful if the referral subsystem is temporarily
    // unavailable. A later approval retry can safely complete the qualification.
    req.log.error({ err: error, driverId }, "Referral qualification after driver approval failed");
  }

  try {
    await closeContestsForApprovedDriverThreshold(user.wilaya, user.commune);
  } catch (error) {
    req.log.warn({ err: error, driverId }, "Failed to close no-driver contests after approved-driver threshold");
  }
...
router.get("/admin/payments", async (req, res): Promise<void> => {
  ...
  const rows = await db
    .select({
      ...
      couponDiscountPercentage: couponsTable.discountPercentage,
      couponMaxDiscountAmount: couponsTable.maxDiscountAmount,
      couponAppliedAmountDzd: couponsTable.appliedAmountDzd,
    })
    .from(subscriptionPaymentsTable)
    .leftJoin(usersTable, eq(subscriptionPaymentsTable.driverId, usersTable.id))
    .leftJoin(couponsTable, eq(couponsTable.appliedToPaymentId, subscriptionPaymentsTable.id))
    .where(
      status && status !== "all"
...
router.post("/admin/payments/:paymentId/approve", async (req, res): Promise<void> => {
  ...
  // Receipt approvals must keep the paid renewal duration; the first-approval
  // 90-day trial is granted only by the driver approval endpoint above.
  ...
  const txResult = await db.transaction(async (tx) => {
    const [previousPaidPayment] = await tx
      .select({ id: subscriptionPaymentsTable.id })
      .from(subscriptionPaymentsTable)
      .where(and(
        eq(subscriptionPaymentsTable.driverId, payment.driverId),
        eq(subscriptionPaymentsTable.status, "approved"),
      ))
      .limit(1);
    ...
    // The first approved paid renewal starts the 60-day window for every
    // unactivated coupon belonging to this driver. The update is conditional
    // so a concurrent approval cannot reset an existing trigger.
    if (!previousPaidPayment) {
      await tx
        .update(couponsTable)
        .set({
          activationTriggerAt: sql`COALESCE(${couponsTable.activationTriggerAt}, NOW())`,
          expiresAt: sql`COALESCE(${couponsTable.activationTriggerAt}, NOW()) + INTERVAL '60 days'`,
        })
        .where(and(
          eq(couponsTable.userId, payment.driverId),
          isNull(couponsTable.activationTriggerAt),
          isNull(couponsTable.usedAt),
        ));
    }

    // A coupon is consumed only when the admin approves the payment, not
    // when the receipt is uploaded. Store the exact amount applied at the
    // same time as used_at so future subscription-price changes cannot alter
    // the historical value.
    const [reservedCoupon] = await tx
      .select({
        id: couponsTable.id,
        discountPercentage: couponsTable.discountPercentage,
        maxDiscountAmount: couponsTable.maxDiscountAmount,
        usedAt: couponsTable.usedAt,
      })
      .from(couponsTable)
      .where(eq(couponsTable.appliedToPaymentId, paymentId))
      .limit(1);

    if (reservedCoupon) {
      if (reservedCoupon.usedAt) {
        throw new Error("COUPON_ALREADY_USED");
      }

      // IMPORTANT: applied_amount_dzd is the platform's discount cost
      // (the applied discount amount), not the final remaining price paid
      // by the user.
      const appliedAmountDzd = discountAmountFor(
        reservedCoupon.discountPercentage,
        reservedCoupon.maxDiscountAmount,
      );
      const [consumedCoupon] = await tx
        .update(couponsTable)
        .set({ usedAt: now, appliedAmountDzd })
        .where(and(
          eq(couponsTable.id, reservedCoupon.id),
          eq(couponsTable.appliedToPaymentId, paymentId),
          isNull(couponsTable.usedAt),
        ))
...
router.post("/admin/payments/:paymentId/reject", async (req, res): Promise<void> => {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      .update(couponsTable)
      .set({ appliedToPaymentId: null })
      .where(and(
        eq(couponsTable.appliedToPaymentId, paymentId),
        sql`${couponsTable.usedAt} IS NULL`,
      ));
  });

...
router.post("/ratings/:ratingId/dispute", async (req, res): Promise<void> => {
  ...
  if (existing.ratedUserId !== req.auth!.userId) {
    res.status(403).json({ error: "غير مصرح لك بفتح نزاع على هذا التقييم" });
    return;
  }
...
router.get("/orders/:orderId/rating", async (req, res): Promise<void> => {
  ...
  const [order] = await db
    .select({ userId: ordersTable.userId, driverId: ordersTable.driverId })
    .from(ordersTable)
    .where(eq(ordersTable.id, orderId));

  if (!order || (order.userId !== req.auth!.userId && order.driverId !== req.auth!.userId)) {
    res.status(403).json({ error: "غير مصرح لك بالاطلاع على تقييم هذا الطلب" });
    return;
  }

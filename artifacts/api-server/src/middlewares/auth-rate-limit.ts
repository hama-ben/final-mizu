...
const _ipLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    ...
    logger.warn(
      { ip, path: req.path, method: req.method },
      "IP-wide auth rate limit triggered"
    );
    res.status(429).json({
      error: "طلبات كثيرة جداً. يرجى المحاولة مجدداً بعد 15 دقيقة.",
      code: "RATE_LIMITED",
...
export function authRateLimiter(req: Request, res: Response, next: NextFunction): void {
  const runIpLimiter = (): void => {
    try {
      _ipLimiter(req, res, (err?: unknown) => {
        if (err) {
          logger.warn({ err }, "IP-wide rate limiter internal error — failing open");
        }
        next();
      });
    } catch (err) {
      logger.warn({ err }, "IP-wide rate limiter threw synchronously — failing open");
      next();
    }
  };

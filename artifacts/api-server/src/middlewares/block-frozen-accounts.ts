...
import { db, usersTable, driverDetailsTable } from "@workspace/db";
...
async function fetchAccountStatus(userId: string, isDriver: boolean): Promise<string | null> {
  const cached = statusCache.get(userId);
  if (cached && cached.expiresAt > Date.now()) {
    return cached.driverSuspended ? "suspended" : cached.status;
  }
  ...
  if (isDriver) {
    ...
    driverSuspended = driverDetails?.isSuspended === true;
  }
  ...
  return driverSuspended ? "suspended" : user.accountStatus;
}
...
export async function blockFrozenAccounts(
  ...
  try {
    const status = await fetchAccountStatus(userId, req.auth?.userType === "سائق");

...
CREATE TABLE IF NOT EXISTS "users" (
  ...
  "first_approval_granted"  boolean     NOT NULL DEFAULT false,
  "referral_code"           text,
...
CREATE TABLE IF NOT EXISTS "referral_rewards" (
  ...
  CONSTRAINT "referral_rewards_referrer_milestone_unique" UNIQUE("referrer_id", "milestone")
);

CREATE TABLE IF NOT EXISTS "referral_rewards_pending_admin" (
  "id"                uuid        PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
  ...
  CONSTRAINT "referral_rewards_pending_admin_driver_threshold_unique"
    UNIQUE("driver_id", "threshold_reached")
);
...
-- Referral rows are written only by the API server. No public or authenticated
-- Supabase policy is created for this table; the server's database role bypasses
-- RLS while browser clients cannot read or write it directly.
ALTER TABLE "referrals" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "referral_rewards" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "referral_rewards_pending_admin" ENABLE ROW LEVEL SECURITY;
...
CREATE TABLE IF NOT EXISTS "monthly_driver_awards" (
  "id"                         uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
  "driver_id"                  text NOT NULL REFERENCES "users"("id"),
  ...
  "average_stars"              numeric NOT NULL,
  "average_delivery_seconds"   numeric NOT NULL,
  "wheel_spin_id"              uuid,
  "created_at"                 timestamp NOT NULL DEFAULT now(),
  CONSTRAINT "monthly_driver_awards_region_month_unique"
    UNIQUE("wilaya", "commune", "month_start")
);
...
CREATE TABLE IF NOT EXISTS "coupons" (
  ...
  "activation_trigger_at"   timestamptz,
  "expires_at"               timestamptz,
  "used_at"                  timestamptz,
  "applied_to_payment_id"   text,
  "applied_amount_dzd"      integer
);

-- Spin/coupon records are server-only, just like referrals.
...
CREATE TABLE IF NOT EXISTS "orders" (
  ...
  "stale_notice_sent_at" timestamp
);
...
ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "no_driver_contest_used" boolean NOT NULL DEFAULT false;
ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "no_driver_contest_started_at" timestamp;
ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "no_driver_contest_rewarded_at" timestamp;
ALTER TABLE "orders" ADD COLUMN IF NOT EXISTS "order_type" text NOT NULL DEFAULT 'normal';
ALTER TABLE "orders" ADD COLUMN IF NOT EXISTS "stale_notice_sent_at" timestamp;
ALTER TABLE "orders" ADD COLUMN IF NOT EXISTS "accepted_at" timestamp;
ALTER TABLE "orders" ADD COLUMN IF NOT EXISTS "delivered_at" timestamp;
ALTER TABLE "monthly_driver_awards" ENABLE ROW LEVEL SECURITY;

-- Existing approved drivers have already passed their first approval. Mark
-- them as handled so the 90-day trial cannot be granted on a later reapproval.
UPDATE "users"
SET "first_approval_granted" = true
WHERE "user_type" = 'سائق'
  AND "account_status" = 'approved'
  AND "first_approval_granted" = false;
...
-- Coupon accounting: preserve the historical amount actually applied at approval.
ALTER TABLE "coupons" ADD COLUMN IF NOT EXISTS "applied_amount_dzd" integer;

-- The payment table uses text IDs. Align the legacy coupon reference before
-- adding the real foreign key to subscription_payments.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'applied_to_payment_id'
      AND data_type <> 'text'
  ) THEN
    ALTER TABLE "coupons"
      ALTER COLUMN "applied_to_payment_id" TYPE text
      USING "applied_to_payment_id"::text;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'coupons_applied_to_payment_id_fkey'
      AND conrelid = 'public.coupons'::regclass
  ) THEN
    ALTER TABLE "coupons"
      ADD CONSTRAINT "coupons_applied_to_payment_id_fkey"
      FOREIGN KEY ("applied_to_payment_id")

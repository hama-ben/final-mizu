...
export function useRealtimeOrders(driverId: string, currentOrderCount: number) {
  ...
  const handleOrderStale = useCallback(() => {
    queryClient.invalidateQueries({
      queryKey: getGetActiveOrdersQueryKey({ driverId }),
    });
  }, [driverId, queryClient]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      socket.on("order_stale", handleOrderStale);
    } catch (err) {
    ...
    return () => {
      if (socket) {
        try {
          ...
          socket.off("order_stale", handleOrderStale);
        } catch {
  ...
  useEffect(() => {
    ...
    try {
      channel = supabase
        ...
        .on("broadcast", { event: "order_stale" }, handleOrderStale)
  ...
  }, [handleNewOrder, handleOrderClaimed, handleOrderStale]);

...
// Now delegates to the shared AppealOverlay component (also used by
// BannedAccountOverlay in App.tsx).  The appeal form calls /api/appeal
// (the generic endpoint) instead of the old /api/driver/appeal.
// ─────────────────────────────────────────────────────────────────────────────
function RejectedAccountOverlay() {
  return (
    <AppealOverlay
      title="تم رفض طلبك"
      idleDescription="الرجاء التواصل مع الإدارة عبر الصفحات الرسمية"
...
function DriverDashboardContent({ driverId }: { driverId: string }) {
  ...
  const [, setLocation] = useLocation();
  const [monthlyAwardNotice, setMonthlyAwardNotice] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const handleMonthlyAward = (payload: { message?: string }) => {
      setMonthlyAwardNotice(payload.message || "مبروك! أنت سائق الشهر في بلديتك 🏆");
    };

    socket.on("monthly_driver_award", handleMonthlyAward);
    return () => {
      socket.off("monthly_driver_award", handleMonthlyAward);
    };
  }, []);
      ...
      {!isRejected && !isPending && isGraceActive && (
        <div className="mx-4 mt-4 glass-panel rounded-2xl p-4 border-2 border-amber-300/60 bg-amber-50/70 dark:bg-amber-900/20 flex items-start gap-3">
          ...
          <p className="text-sm text-amber-800 dark:text-amber-300 leading-relaxed">
            وصل اشتراكك قيد المراجعة من الإدارة — يمكنك استقبال الطلبيات مؤقتاً في هذه الأثناء، لكن هذا لا يُحتسب كأيام فعلية من رصيدك. سيُضاف رصيدك الحقيقي فور قبول الوصل.
          </p>
        </div>
      )}
      {monthlyAwardNotice && (
        <div
          className="mx-4 mt-4 rounded-3xl border-2 border-amber-300 bg-gradient-to-l from-amber-50 to-yellow-50 p-4 text-amber-900 shadow-lg shadow-amber-200/50 dark:border-amber-700 dark:from-amber-900/30 dark:to-yellow-900/20 dark:text-amber-200"
          dir="rtl"
          role="status"
        >
          <div className="flex items-start gap-3">
            <Trophy className="mt-0.5 h-6 w-6 shrink-0 text-amber-500" />
            <p className="flex-1 font-black">{monthlyAwardNotice}</p>
            <button
              onClick={() => setMonthlyAwardNotice(null)}
          ...
          <p className="mt-2 pr-9 text-sm text-amber-800/80 dark:text-amber-300/80">
            تمت إضافة لفة عجلة حظ واحدة إلى رصيدك.
          </p>
        </div>
      )}
...
// • ترسل الإحداثيات إلى Supabase كل 5 ثوانٍ — فقط عندما تكون الحالة "قيد التوصيل"
// ─────────────────────────────────────────────────────────────────────────────
function SatelliteMap({
  orderId, driverId, destLat, destLng, orderStatus,
}: {
  orderId: string;
  driverId: string;
  destLat: number;
  destLng: number;
  /** Only broadcasts location to Supabase while this equals "قيد التوصيل". */
  orderStatus: string;
}) {
  ...
  // Use a ref so the watchPosition callback always reads the *current* status
  // without needing to re-mount the map when the order moves from
  // "قيد التوصيل" → "وصل السائق".
    ...
    const initMap = () => {
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          // إرسال الإحداثيات إلى Supabase كل 5 ثوانٍ — فقط أثناء التوصيل الفعلي.
          // نتوقف عن الإرسال حين تصبح الحالة "وصل السائق" أو "تم التوصيل"
          // حتى لا نُبقي موقع السائق مكشوفاً أطول من اللازم.
          const now = Date.now();
          if (
            orderStatusRef.current === "قيد التوصيل" &&
            now - lastSupabaseUpdate.current > 5000
          ) {
...
const FAVORITE_ORDER_WINDOW_SECONDS = 90;
...
function PendingOrdersQueue({ driverId }: { driverId: string }) {
  ...
  const favoriteOrderDeadlinesRef = useRef<Record<string, number>>({});
  const [countdownNow, setCountdownNow] = useState(() => Date.now());

  // Start each favourite-order countdown only the first time that order is
  // observed. This ref survives the six-second query refreshes, so polling
  // cannot reset a countdown back to 01:30.
  useEffect(() => {
    const visibleFavoriteOrderIds = new Set(
      (orders ?? [])
        .filter((order) => order.isFavoriteConsumer)
        .map((order) => order.id),
    );

    for (const orderId of visibleFavoriteOrderIds) {
      if (!favoriteOrderDeadlinesRef.current[orderId]) {
        favoriteOrderDeadlinesRef.current[orderId] =
          Date.now() + FAVORITE_ORDER_WINDOW_SECONDS * 1000;
      }
    }

    for (const orderId of Object.keys(favoriteOrderDeadlinesRef.current)) {
      if (!visibleFavoriteOrderIds.has(orderId)) {
        delete favoriteOrderDeadlinesRef.current[orderId];
      }
    }
  }, [orders]);
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        {!orders || orders.length === 0 ? (
          ...
          orders.map(order => {
            const isStale = (order as typeof order & { isStale?: boolean }).isStale === true;
            const favoriteDeadline = favoriteOrderDeadlinesRef.current[order.id];
            const favoriteRemainingSeconds = order.isFavoriteConsumer
              ? Math.max(
                  0,
                  Math.ceil(
                    ((favoriteDeadline ?? (countdownNow + FAVORITE_ORDER_WINDOW_SECONDS * 1000)) - countdownNow) / 1000,
                  ),
                )
              : 0;
            const favoriteMinutes = String(Math.floor(favoriteRemainingSeconds / 60)).padStart(2, "0");
            const favoriteSeconds = String(favoriteRemainingSeconds % 60).padStart(2, "0");

            return (
            <div
              key={order.id}
              className={`glass-panel p-5 rounded-3xl overflow-hidden relative ${
                order.isFavoriteConsumer
                  ? "favorite-order-glow border-2 border-rose-500 dark:border-rose-400 shadow-xl shadow-rose-300/50 dark:shadow-rose-900/40"
                  : isStale
                    ? "border-2 border-orange-400 dark:border-orange-500 shadow-xl shadow-orange-200/60 dark:shadow-orange-900/30"
                  : "border border-transparent"
              }`}
              data-testid={`pending-order-${order.id}`}
            >
              <div className={`absolute top-0 right-0 h-full ${order.isFavoriteConsumer ? "w-2 bg-rose-500" : isStale ? "w-2 bg-orange-500" : "w-1 bg-sky-400"}`} />

              {order.isFavoriteConsumer && (
                <div className="flex items-center justify-between gap-3 mb-4 rounded-2xl border border-rose-200 dark:border-rose-800 bg-rose-50 dark:bg-rose-900/25 px-4 py-3" dir="rtl">
                  <div className="flex items-center gap-2 text-rose-700 dark:text-rose-300">
                    <Star className="w-5 h-5 fill-rose-500 text-rose-500" />
                    <span className="text-base font-black">مفضّل</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-rose-700 dark:text-rose-300" aria-label={`الوقت المتبقي ${favoriteMinutes}:${favoriteSeconds}`}>
                    <Timer className="w-5 h-5" />
                    <span className="font-mono text-lg font-black tabular-nums">
                      {favoriteMinutes}:{favoriteSeconds}
                    </span>
                  </div>
                </div>
              )}
              {isStale && !order.isFavoriteConsumer && (
                <div className="flex items-center gap-2 mb-4 rounded-2xl border border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/25 px-4 py-3 text-orange-800 dark:text-orange-300" dir="rtl">
                  <AlertTriangle className="w-5 h-5 shrink-0" />
                  <span className="text-sm font-black">طلب متأخر — انتبه لهذا الطلب</span>
                </div>
              )}

...
import { customFetch, useVerifyOtp, useSubmitDriverDocs } from "@workspace/api-client-react";
import {
  ...
  CreditCard as CreditCardIcon, Sparkles,
  PartyPopper, Timer, Gift, QrCode, ScanLine,
} from "lucide-react";
...
function RegistrationForm({ onOtpSent }: { onOtpSent: (email: string) => void }) {
  ...
  useEffect(() => {
    try {
      ...
      setIsSocialShare(params.get("social_share") === "1");
      ...
      if (draft.referralCode) setReferralCode(draft.referralCode);
        ...
        if (fromLink) setReferralCode(extractReferralCode(fromLink));
      }
    } catch { /* ignore */ }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(REG_DRAFT_KEY, JSON.stringify({
        name, email, phone, wilaya, commune, userType, referralCode,
      }));
    } catch { /* ignore */ }
  }, [name, email, phone, wilaya, commune, userType, referralCode]);
  ...
  useEffect(() => {
    ...
    if (!/^[A-Z0-9]{6,8}$/.test(normalized)) {
      setReferralValidation({ status: "invalid", message: "استخدم من 6 إلى 8 أحرف أو أرقام" });
      return;
    }
    ...
    const timer = window.setTimeout(() => {
      const typeQuery = userType ? `&userType=${encodeURIComponent(userType)}` : "";
      customFetch<{ valid: boolean; error?: string }>(
        `/api/auth/referral-code/validate?code=${encodeURIComponent(normalized)}${typeQuery}`,
      ).then((result) => {
        if (!active) return;
        setReferralValidation(result.valid
          ? { status: "valid", message: "رمز الدعوة صالح" }
          : { status: "invalid", message: result.error || "رمز الدعوة غير صالح" });
      }).catch(() => {
        if (active) setReferralValidation({ status: "idle" });
      });
    }, 350);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (normalizedReferralCode && referralValidation.status === "invalid") {
      setError(referralValidation.message || "رمز الدعوة غير صالح");
      return;
    }
    if (normalizedReferralCode && referralValidation.status === "checking") {
      setError("انتظر حتى يتم التحقق من رمز الدعوة");
      return;
    }
    const normalizedEmail = email.trim().toLowerCase();
    setRequesting(true);
    try {
      await customFetch("/api/auth/register-request", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name, email: normalizedEmail, password, phone, userType, wilaya, commune,
          ...(normalizedReferralCode ? { referralCode: normalizedReferralCode } : {}),
          ...(normalizedReferralCode && isSocialShare ? { referralSource: "social_share" } : {}),
        }),
      });
      ...
      setError(response?.data?.error || response?.message || "حدث خطأ أثناء التسجيل");
    } finally {
      setRequesting(false);
    }
  };
  ...
  return (
    <>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              autoComplete="off"
              spellCheck={false}
              className={`w-full bg-white/50 dark:bg-black/50 border rounded-2xl py-3 pr-10 pl-24 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all uppercase tracking-widest ${
            ...
            <button
              ...
              className="absolute inset-y-0 left-2 my-1.5 px-2.5 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 transition-colors"
              title="مسح رمز QR"
              aria-label="مسح رمز QR"
              data-testid="button-scan-referral"
            >
              <ScanLine className="w-5 h-5" />
            </button>
            {referralValidation.status === "checking" && (
              <Loader2 className="absolute left-14 inset-y-0 my-auto w-4 h-4 text-primary animate-spin" />
            )}
          </div>
          {referralValidation.message && (
            <p className={`text-xs px-1 -mt-2 ${
              referralValidation.status === "valid" ? "text-emerald-600" : "text-destructive"
            }`}>
              {referralValidation.message}
            </p>
          )}
          ...
          <button type="submit" disabled={requesting || !userType}
      ...
      {showScanner && (
        <ReferralScannerModal
          onClose={() => setShowScanner(false)}
          onDetected={(value) => {
            setReferralCode(extractReferralCode(value));
...
function extractReferralCode(value: string): string {
  try {
    ...
    const linkedCode = url.searchParams.get("ref");
    if (linkedCode) return linkedCode.trim().toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 8);
  } catch { /* plain code */ }
  return value.trim().toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 8);
}

function ReferralScannerModal({ onClose, onDetected }: {
  onClose: () => void;
  ...
  useEffect(() => {
    ...
    const BarcodeDetectorClass = (window as unknown as { BarcodeDetector?: DetectorConstructor }).BarcodeDetector;

    const start = async () => {
      if (!BarcodeDetectorClass) {
        setScannerError("المتصفح لا يدعم قراءة QR بالكاميرا. يمكنك إدخال الرمز يدوياً.");
        return;
      }
      if (!navigator.mediaDevices?.getUserMedia) {
        setScannerError("لا يمكن الوصول إلى الكاميرا من هذا المتصفح.");
        return;
      }
      try {
        ...
        const scan = async () => {
          ...
          try {
            ...
            // Keep the camera preview running; a frame can be undecodable.
          }
          frame = window.requestAnimationFrame(scan);
        };
        frame = window.requestAnimationFrame(scan);
      } catch {
        setScannerError("تعذر فتح الكاميرا. اسمح بالوصول إليها ثم حاول مرة أخرى.");
      }
    };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" dir="rtl">
      <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="font-black text-slate-800 dark:text-white">مسح رمز الدعوة</h2>
            <p className="text-xs text-slate-400 mt-1">وجّه الكاميرا نحو رمز QR</p>
          </div>
        ...
        <div className="relative aspect-square overflow-hidden rounded-2xl bg-slate-950">
          ...
          <div className="absolute inset-10 border-2 border-white/80 rounded-2xl shadow-[0_0_0_999px_rgba(0,0,0,0.25)] pointer-events-none" />
        </div>
        {scannerError && (
          <p className="mt-4 rounded-2xl bg-amber-50 dark:bg-amber-900/20 p-3 text-xs text-amber-700 dark:text-amber-300 text-center leading-relaxed">
            {scannerError}
          </p>
        )}
        <button onClick={onClose} className="w-full mt-4 py-3 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold">
          إدخال الرمز يدوياً
        </button>
      </div>
    </div>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500" dir="rtl">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-7 flex flex-col items-center text-center gap-4 animate-in zoom-in-95 duration-300">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
              ...
              <span className="font-bold text-primary">90 يوماً تجريبياً (3 أشهر)</span>{" "}
            ...
            <div className="flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-2xl px-5 py-3">
              ...
              <span className="text-2xl font-black text-primary tracking-wide">90 يوماً</span>
      ...
      <div className="glass-panel rounded-3xl p-4 mb-5 bg-amber-50/50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800">
        <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed font-medium">
          سيتم مراجعة وثائقك من قِبل الإدارة. ستحصل على 90 يوماً تجريبية (3 أشهر) فور الإرسال.
        </p>
      </div>
...
function SubscriptionGate({ session }: { session: VerifiedSession }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500" dir="rtl">
      ...
      <div className="space-y-4">
        {/* Automatic free gift information */}
        <div className="w-full glass-panel rounded-3xl p-6 text-right border-2 border-emerald-300 dark:border-emerald-700">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="font-black text-slate-800 dark:text-white text-lg">هدية مجانية</span>
                <span className="bg-emerald-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">مجاناً</span>
              </div>
              <p className="text-sm text-slate-500 leading-relaxed">
                بعد اعتماد مستنداتك من الإدارة، ستحصل تلقائياً على 90 يوماً مجاناً
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="bg-primary/10 text-primary text-xs font-bold px-2 py-0.5 rounded-full">1500 دج / شهر</span>
              </div>
              <p className="text-sm text-slate-500 leading-relaxed">

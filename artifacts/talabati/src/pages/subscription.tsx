...
function SubscriptionContent({ driverId }: { driverId: string }) {
  ...
  useEffect(() => {
    customFetch<SubscriptionCoupon[]>("/api/coupons")
      .then((data) => {
        setCoupons(data);
        const fromUrl = new URLSearchParams(window.location.search).get("couponId");
        if (fromUrl && data.some((coupon) =>
          coupon.id === fromUrl &&
          (coupon.status === "active" || coupon.status === "pending_activation") &&
          !coupon.appliedToPaymentId
        )) {
          setSelectedCouponId(fromUrl);
        }
      })
      .catch(() => {});
  }, [driverId]);
  ...
  const handleSubmit = () => {
    ...
    submitMutation.mutate(
      {
        driverId,
        // The generated client predates the phase-2 optional field. It is
        // still sent as JSON and validated again by the server transaction.
        data: {
      ...
      },
      {
        onSuccess: () => {
          ...
          customFetch<SubscriptionCoupon[]>("/api/coupons").then(setCoupons).catch(() => {});
  ...
  const selectedCoupon = usableCoupons.find((coupon) => coupon.id === selectedCouponId) ?? null;
      ...
      <div className="glass-panel rounded-3xl p-6 border border-primary/20">
          ...
          <div className="flex justify-between items-center">
            <span className="text-slate-600 dark:text-slate-400">قيمة الاشتراك</span>
             <span className="font-black text-primary text-lg">
               {selectedCoupon ? `${selectedCoupon.amountToPay} دج` : "1500 دج"}
             </span>
          </div>
          <div className="flex justify-between items-center">
        ...
        <div className="mt-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-2xl p-4 space-y-3">
          ...
          <p className="text-slate-700 dark:text-slate-200 text-sm leading-loose">
             قم بتحويل مبلغ <strong>{selectedCoupon ? selectedCoupon.amountToPay : 1500} دج</strong> عبر البريد الجزائري (CCP) إلى الحساب التالي:
          </p>
       ...
       {usableCoupons.length > 0 && (
         <div className="glass-panel rounded-3xl p-5 border border-violet-200 dark:border-violet-800/50" dir="rtl">
           <div className="flex items-center justify-between gap-3 mb-3">
             <div>
               <h2 className="font-bold text-slate-800 dark:text-white">استخدم إحدى قسائمك</h2>
               <p className="text-xs text-slate-500 mt-1">اختيار القسيمة لا يبدأ صلاحيتها؛ تُستهلك فقط بعد قبول الوصل.</p>
             </div>
             <button onClick={() => setLocation("/coupons")} className="text-xs text-violet-600 font-bold hover:underline">عرض الكل</button>
           </div>
               ...
               >
                 <div className="flex items-center justify-between gap-3">
                   <span className="font-bold text-slate-800 dark:text-white">
                     {coupon.discountPercentage === 100 ? "شهر مجاني بالكامل" : `خصم ${coupon.discountPercentage}%`}
                   </span>
                   <span className="font-black text-violet-600">{coupon.amountToPay} دج</span>
                 </div>
                 {coupon.maxDiscountAmount && <p className="text-[11px] text-slate-500 mt-1">بحد أقصى {coupon.maxDiscountAmount} دج</p>}
               </button>
             ))}
           </div>
           {selectedCoupon && (
             <p className="mt-3 text-xs text-violet-700 dark:text-violet-300 font-bold">
               تم اختيار القسيمة. ارفع وصلاً بالمبلغ المخفّض أعلاه.
             </p>
           )}
         </div>
       )}
        ...
        <div className="glass-panel rounded-3xl p-6 text-center border border-amber-200 dark:border-amber-700 space-y-3">
          ...
          <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700 rounded-2xl p-3 flex items-start gap-2 text-right">
            ...
            <p className="text-sm text-emerald-700 dark:text-emerald-300 font-medium">
              مكافأة: تم منحك <strong>90 يوماً مجانية (3 أشهر)</strong> فور رفع الوصل. يمكنك استقبال الطلبات الآن بينما يُراجع الإداريون وصلك.
            </p>
          </div>
        </div>
      ) : (
